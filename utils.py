...
def find_intersection(set_1, set_2):
    ...
    intersection_dims = torch.clamp(upper_bounds - lower_bounds, min=0)  # (n1, n2, 2) #обрубает все, что меньше 0
...
def transform(image, boxes, labels, difficulties, split):
    ...
    if(split == 'TRAIN'):
        ...
        if(random.random() < 0.5):
            new_image, new_boxes = expand(new_image, boxes, filler=mean) # не понял, как это работает
...
def photometric_distort(image):
    ...
    distortions = [FT.adjust_brightness,    #яркость
                   FT.adjust_contrast,      #контраст
                   FT.adjust_saturation,    #насыщение
                   FT.adjust_hue]           #шум
